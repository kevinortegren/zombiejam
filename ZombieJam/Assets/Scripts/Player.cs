...
public class Player : controller {
	...
	void Start () {
		...
		foreach (Transform t in joints)
		{
			if (t.name == "RightHand")
			{
                ...
                if (t.GetChild(0).name == "Blaster(Clone)")
                {
                    weaponTransform = t.GetChild(0).GetChild(0);
	...
	void RotateArm()
	{
		if (aimVec != Vector3.zero)
		{
			...
			if (activeWeapon.name == "Hammer")
			{
				hand.localRotation = Quaternion.Euler(new Vector3(90, 90, 90));
			}
			if (activeWeapon.name == "Blaster")
			{
				hand.localRotation = Quaternion.Euler(new Vector3(90, 90, 90));
			}
		}
	...
	void Shoot()
	{
		...
		if (aimVec != Vector3.zero)
		{
			if(Time.time >= timeWhenFired + weapon.cooldown)
			{
				...
				weapon.Fire(gameObject, weaponTransform.position, new Vector2(latestAimingDirection.x, latestAimingDirection.y));		
